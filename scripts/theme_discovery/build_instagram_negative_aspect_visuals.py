...
def build_period_summary() -> pd.DataFrame:
    df = pd.read_csv(ROOT / "instagram_negative_aspect_theme_mapping.csv")
    summary = (
        df.groupby(["theme_code", "theme_label", "time_bucket"], as_index=False)
        .size()
        .rename(columns={"size": "matched_negative_clause_count"})
    )
    summary.to_csv(ROOT / "instagram_negative_aspects_period_summary.csv", index=False)
    return summary


def plot_overall() -> None:
    df = pd.read_csv(ROOT / "instagram_negative_aspects_by_theme.csv")
    ...
    ax.set_title("Distribusi Negative Aspect Instagram Comments")
    ...
    save_current_figure(FIGURES_DIR / "fig_17_instagram_negative_aspects_overall.svg")
...
def plot_by_period() -> None:
    ...
    ax.set_title("Distribusi Negative Aspect Instagram Comments per Periode")
    ...
    save_current_figure(FIGURES_DIR / "fig_18_instagram_negative_aspects_by_period.svg")


def plot_audit() -> None:
    df = pd.read_csv(ROOT / "instagram_negative_aspect_audit.csv")
    key_order = [
        "negative_comments_detected",
    ...
    label_map = {
        "negative_comments_detected": "Neg Comments",
    ...
    ax.set_title("Audit Negative Aspect Instagram Comments")
    ax.set_xlabel("")
    ...
    save_current_figure(FIGURES_DIR / "fig_19_instagram_negative_aspect_audit.svg")


def write_summary() -> None:
    summary = "\n".join(
        [
            "# Instagram Comments Negative Aspect Visuals",
            "",
            "Visualisasi ini dibuat dari hasil mapping negative aspect Instagram comments berbasis clause atau comment negatif.",
            "",
            "## File Figure",
            "- `fig_17_instagram_negative_aspects_overall.svg`",
            "- `fig_18_instagram_negative_aspects_by_period.svg`",
            "- `fig_19_instagram_negative_aspect_audit.svg`",
            "",
            "## File Tabel Pendukung",
            "- `instagram_negative_aspects_by_theme.csv`",
            "- `instagram_negative_aspects_period_summary.csv`",
            "- `instagram_negative_aspect_audit.csv`",
        ]
    )
    (ROOT / "instagram_negative_aspect_visuals_summary.md").write_text(summary, encoding="utf-8")
...
def main() -> None:
    ...
    plot_audit()
    write_summary()
    print(f"[negative_aspect_visuals] saved Instagram negative aspect figures under {FIGURES_DIR}")
